...
public final class FunctionProcessor {
    ...
    // 单例实例
    private static volatile FunctionProcessor INSTANCE;

    // 获取单例
    public static FunctionProcessor getInstance() {
        if (INSTANCE == null) {
            synchronized (FunctionProcessor.class) {
                if (INSTANCE == null) {
                    INSTANCE = new FunctionProcessor();
                }
            }
        }
        return INSTANCE;
    ...
    // 内部状态
    private BlockingQueue<LabdatahubFunctionRecord> queue;
    private volatile Thread worker;
    private final AtomicBoolean running = new AtomicBoolean(false);
    private FunctionConsumer consumer;
     ...
     */
    public synchronized void start(int batchSize, int maxQueueSize, FunctionConsumer consumer) {
        if (running.get()) {
            return;
        }
     ...
     * @return 是否成功添加
     */
    public boolean addLog(LabdatahubFunctionRecord log) {
        if (!running.get() || queue == null) {
            return false;
        }
     ...
     * @return 成功添加的数量
     */
    public int addLogs(List<LabdatahubFunctionRecord> logs) {
    ...
    private void consume() {
        List<LabdatahubFunctionRecord> batch = new ArrayList<>(batchSize);
        ...
        while (running.get() && !Thread.currentThread().isInterrupted()) {
            try {
                ...
                // 没有数据时，等待第一条（避免空转）
                LabdatahubFunctionRecord first = queue.poll(100, TimeUnit.MILLISECONDS);
                if (first != null) {
                    batch.add(first);
                    // 立即取剩余数据
    ...
    private void processRemaining() {
        ...
        List<LabdatahubFunctionRecord> remaining = new ArrayList<>();
