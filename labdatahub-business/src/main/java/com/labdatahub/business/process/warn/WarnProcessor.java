 ...
 * 极简高性能日志处理器（单例）
 */
public final class WarnProcessor {

    // 单例实例
    private static volatile WarnProcessor INSTANCE;

    // 获取单例
    public static WarnProcessor getInstance() {
        if (INSTANCE == null) {
            synchronized (WarnProcessor.class) {
                if (INSTANCE == null) {
                    INSTANCE = new WarnProcessor();
                }
            }
        }
        return INSTANCE;
    ...
    // 私有构造
    private WarnProcessor() {
        // 注册JVM关闭钩子
        Runtime.getRuntime().addShutdownHook(new Thread(this::stop));
    }

    // 内部状态
    private BlockingQueue<LabdatahubWarnRecord> queue;
    private volatile Thread worker;
    private final AtomicBoolean running = new AtomicBoolean(false);
    private WarnConsumer consumer;
     ...
     */
    public synchronized void start(int batchSize, int maxQueueSize, WarnConsumer consumer) {
        if (running.get()) {
            return;
        }
     ...
     * @return 是否成功添加
     */
    public boolean addLog(LabdatahubWarnRecord log) {
        if (!running.get() || queue == null) {
            return false;
        }
     ...
     * @return 成功添加的数量
     */
    public int addLogs(List<LabdatahubWarnRecord> logs) {
    ...
    private void consume() {
        List<LabdatahubWarnRecord> batch = new ArrayList<>(batchSize);
        ...
        while (running.get() && !Thread.currentThread().isInterrupted()) {
            try {
                ...
                // 没有数据时，等待第一条（避免空转）
                LabdatahubWarnRecord first = queue.poll(100, TimeUnit.MILLISECONDS);
                if (first != null) {
                    batch.add(first);
                    // 立即取剩余数据
    ...
    private void processRemaining() {
        ...
        List<LabdatahubWarnRecord> remaining = new ArrayList<>();
