 ...
 * 极简高性能日志处理器（单例）
 */
public final class LinkageProcessor {

    // 单例实例
    private static volatile LinkageProcessor INSTANCE;

    // 获取单例
    public static LinkageProcessor getInstance() {
        if (INSTANCE == null) {
            synchronized (LinkageProcessor.class) {
                if (INSTANCE == null) {
                    INSTANCE = new LinkageProcessor();
                }
            }
        }
        return INSTANCE;
    ...
    // 私有构造
    private LinkageProcessor() {
        // 注册JVM关闭钩子
        Runtime.getRuntime().addShutdownHook(new Thread(this::stop));
    }

    // 内部状态
    private BlockingQueue<LabdatahubLinkageWarnRecord> queue;
    private volatile Thread worker;
    private final AtomicBoolean running = new AtomicBoolean(false);
    private LinkageConsumer consumer;
     ...
     */
    public synchronized void start(int batchSize, int maxQueueSize, LinkageConsumer consumer) {
        if (running.get()) {
            return;
        }
     ...
     * @return 是否成功添加
     */
    public boolean addLog(LabdatahubLinkageWarnRecord log) {
        if (!running.get() || queue == null) {
            return false;
        }
        return queue.offer(log); // 队列满时返回false，自动丢弃
    }
     ...
     * @return 成功添加的数量
     */
    public int addLogs(List<LabdatahubLinkageWarnRecord> logs) {
    ...
    private void consume() {
        List<LabdatahubLinkageWarnRecord> batch = new ArrayList<>(batchSize);
        ...
        while (running.get() && !Thread.currentThread().isInterrupted()) {
            try {
                ...
                // 没有数据时，等待第一条（避免空转）
                LabdatahubLinkageWarnRecord first = queue.poll(100, TimeUnit.MILLISECONDS);
                if (first != null) {
                    batch.add(first);
                    // 立即取剩余数据
    ...
    private void processRemaining() {
        ...
        List<LabdatahubLinkageWarnRecord> remaining = new ArrayList<>();
