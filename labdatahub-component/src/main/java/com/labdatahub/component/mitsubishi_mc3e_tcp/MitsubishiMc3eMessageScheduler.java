...
public class MitsubishiMc3eMessageScheduler {
    ...
    public static void addReadConfig(String componentId, MitsubishiMc3eReadConfig readConfig) {
        ...
        ScheduledFuture<?> future = scheduler.scheduleAtFixedRate(
                () -> {
                    ...
                    // 无关；消费线程则由 MitsubishiMc3eConnectionManager#addConnection 创建。两者不同步时
                    ...
                    if (!MitsubishiMc3eLoopConsumer.isConsuming(componentId)) {
                        return;
                    }
                    // 将消息放入对应组件的队列
                    try {
    ...
    private static void logQueueFull(String componentId) {
        ...
        if (lastLogTs == null || now - lastLogTs >= QUEUE_FULL_LOG_INTERVAL_MS) {
            ...
            log.warn("[MC调度] componentId={} 消息队列已满（上限{}），丢弃最旧消息以保证上报的是最新数据",
