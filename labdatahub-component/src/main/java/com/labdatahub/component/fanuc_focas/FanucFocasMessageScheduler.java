...
public class FanucFocasMessageScheduler {
    ...
    public static final Map<String, BlockingQueue<FanucFocasMessage>> messageQueueMap = new ConcurrentHashMap<>();
    ...
    /**
     * 【静态方法】添加FANUC读取配置，启动定时生成消息任务
     * @param componentId 组件ID（标识消息队列所属类型，不能为空）
     * @param readConfig 读取配置（必须包含deviceSn、code、intervalTime）
     */
    public static void addReadConfig(String componentId, FanucFocasReadConfig readConfig) {
        ...
        if (readConfig == null) {
            throw new IllegalArgumentException("FanucFocasReadConfig 不能为null");
        }
        ...
        // 3. 创建定时任务：按intervalTime（秒）循环生成FanucFocasMessage
        ScheduledFuture<?> future = scheduler.scheduleAtFixedRate(
                () -> {
                    // 配置转消息（核心映射逻辑）
                    FanucFocasMessage message = convertToFanucFocasMessage(readConfig);
                    // 根据componentId获取/创建对应的队列（原子操作，线程安全）
                    BlockingQueue<FanucFocasMessage> targetQueue = messageQueueMap.computeIfAbsent(
                            componentId,
        ...
        System.out.printf("已添加FANUC定时配置：componentId=%s, deviceSn=%s, code=%s, 间隔=%d秒%n",
                componentId, readConfig.getDeviceSn(), readConfig.getCode(), readConfig.getIntervalTime());
    }
    /**
     * 【静态方法】移除指定的FANUC读取配置，停止定时生成消息
    ...
    public static void removeReadConfig(String componentId, String deviceSn, String code) {
        ...
        if (future != null) {
            ...
            System.out.printf("已移除FANUC定时配置：componentId=%s, deviceSn=%s, code=%s%n",
     ...
     * @return FanucFocasMessage
     * @throws InterruptedException 线程中断异常
     * @throws IllegalArgumentException componentId为空或无对应队列时抛出
     */
    public static FanucFocasMessage takeMessage(String componentId) throws InterruptedException {
        // 参数校验
        if (StringUtils.isBlank(componentId)) {
            throw new IllegalArgumentException("componentId 不能为空");
        }
        // 获取对应组件的队列（不存在则抛异常）
        BlockingQueue<FanucFocasMessage> targetQueue = messageQueueMap.get(componentId);
        if (targetQueue == null) {
     ...
     * @param componentId 组件ID（标识要读取的队列）
     * @return FanucFocasMessage 或 null
     * @throws IllegalArgumentException componentId为空时抛出
     */
    public static FanucFocasMessage pollMessage(String componentId) {
        // 参数校验
        if (StringUtils.isBlank(componentId)) {
            throw new IllegalArgumentException("componentId 不能为空");
        }
    ...
    public static void shutdown() {
        ...
        System.out.println("FANUC消息调度器已关闭，所有组件队列已清空");
    }
    /**
     * 【静态方法】FanucFocasReadConfig 转 FanucFocasMessage（字段映射）
     */
    private static FanucFocasMessage convertToFanucFocasMessage(FanucFocasReadConfig readConfig) {
        FanucFocasMessage message = new FanucFocasMessage();
