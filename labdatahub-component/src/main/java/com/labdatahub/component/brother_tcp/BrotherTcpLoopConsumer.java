 ...
 */
public class BrotherTcpLoopConsumer {
    ...
    // 消费线程前缀（便于日志排查）
    private static final String CONSUME_THREAD_NAME_PREFIX = "brother-loop-consumer-";
    // ========== 内部消费线程类（封装循环逻辑+停止标志） ==========
    private static class ConsumeThread extends Thread {
        private final String componentId; // 目标组件ID
        private final BrotherTcpMessageConsumeHandler consumeHandler; // 消费回调
        private final AtomicBoolean isRunning = new AtomicBoolean(true); // 运行标志
        public ConsumeThread(String componentId, BrotherTcpMessageConsumeHandler consumeHandler) {
        ...
        public void run() {
            ...
            while (isRunning.get()) {
                try {
                    ...
                    BrotherTcpMessage message = BrotherTcpMessageScheduler.takeMessage(componentId);
                    // 调用业务侧的消费逻辑
     ...
     */
    public static void startConsume(String componentId, BrotherTcpMessageConsumeHandler consumeHandler) {
        // 1. 参数校验
    ...
    public static void stopAllConsume() {
        ...
        System.out.println("所有Brother循环消费线程已停止");
    }
}
