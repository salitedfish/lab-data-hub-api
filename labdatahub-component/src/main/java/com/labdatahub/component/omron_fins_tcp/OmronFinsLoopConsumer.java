 ...
 */
public class OmronFinsLoopConsumer {
    ...
    // 消费线程前缀（便于日志排查）
    private static final String CONSUME_THREAD_NAME_PREFIX = "omronFins-loop-consumer-";

    // ========== 内部消费线程类（封装循环逻辑+停止标志） ==========
    private static class ConsumeThread extends Thread {
        private final String componentId; // 目标组件ID
        private final OmronFinsMessageConsumeHandler consumeHandler; // 消费回调
        private final AtomicBoolean isRunning = new AtomicBoolean(true); // 运行标志

        public ConsumeThread(String componentId, OmronFinsMessageConsumeHandler consumeHandler) {
        ...
        public void run() {
            ...
            while (isRunning.get()) {
                try {
                	...
                	OmronFinsMessage message = OmronFinsMessageScheduler.takeMessage(componentId);
                    // 调用业务侧的消费逻辑
     ...
     */
    public static void startConsume(String componentId, OmronFinsMessageConsumeHandler consumeHandler) {
        // 1. 参数校验
    ...
    public static void stopAllConsume() {
    	CONSUME_THREAD_MAP.keySet().forEach(OmronFinsLoopConsumer::stopConsume);
