...
public class ModbusMessageScheduler {
    ...
    public static void addReadConfig(String componentId, ModbusReadConfig readConfig) {
        ...
        ScheduledFuture<?> future = scheduler.scheduleAtFixedRate(
                () -> {
                    ...
                    // 无关；消费线程则由 ModbusConnectionManager#addConnection 创建。两者不同步时
                    ...
                    if (!ModbusLoopConsumer.isConsuming(componentId)) {
                        return;
                    }
                    // 将消息放入对应组件的队列
                    try {
    ...
    private static void logQueueFull(String componentId) {
        ...
        if (lastLogTs == null || now - lastLogTs >= QUEUE_FULL_LOG_INTERVAL_MS) {
            ...
            log.warn("[Modbus调度] componentId={} 消息队列已满（上限{}），丢弃最旧消息以保证上报的是最新数据",
